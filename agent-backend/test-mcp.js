...
const mcp_service_1 = require("./src/services/mcp.service");
...
async function testMCP() {
    ...
    try {
        ...
        const newCustomer = await mcp.createCustomer('Test User ' + timestamp, `test${timestamp}@example.com`, '+1-555-' + Math.floor(Math.random() * 1000000));
        ...
        testCustomerId = newCustomer.customerId || newCustomer.id; // Use this for later tests
    ...
    if (testCustomerId) {
        try {
            ...
            const updatedCustomer = await mcp.updateCustomer(testCustomerId, 'Updated Test User', `updated${Date.now()}@example.com`, '+1-555-9999999');
    ...
    if (testCustomerId) {
        try {
            ...
            const newCard = await mcp.createCard(testCustomerId, cardNumber, 'debit', '12/2026');
    ...
    if (testCardId) {
        try {
            ...
            const cardNumber = '4532' + Math.floor(Math.random() * 1000000000000).toString().padStart(12, '0');
            const updatedCard = await mcp.updateCard(testCardId, cardNumber, 'credit', '12/2027');
